...
CTexture2::CTexture2(int nTextures, UINT nTextureType, int nSamplers, int nGraphicsSrvRootParameters, int nComputeUavRootParameters, int nComputeSrvRootParameters)
{
	m_nTextureType = nTextureType;
...
void CTexture2::SetSrvGpuDescriptorHandle(int nIndex, D3D12_GPU_DESCRIPTOR_HANDLE d3dSrvGpuDescriptorHandle)
{
	m_pd3dSrvGpuDescriptorHandles[nIndex] = d3dSrvGpuDescriptorHandle;
}

void CTexture2::SetUavGpuDescriptorHandle(int nIndex, D3D12_GPU_DESCRIPTOR_HANDLE d3dUavGpuDescriptorHandle)
{
	m_pd3dUavGpuDescriptorHandles[nIndex] = d3dUavGpuDescriptorHandle;
}

void CTexture2::SetGraphicsSrvRootParameterIndex(int nIndex, int nRootParameterIndex, int nGpuHandleIndex)
{
...
void CTexture2::SetComputeUavRootParameterIndex(int nIndex, int nRootParameterIndex, int nGpuHandleIndex)
{
...
void CTexture2::SetComputeSrvRootParameterIndex(int nIndex, int nRootParameterIndex, int nGpuHandleIndex)
{
...
void CTexture2::UpdateComputeShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
...
void CTexture2::UpdateGraphicsShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
...
void CTexture2::LoadTextureFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, wchar_t* pszFileName, UINT nResourceType, UINT nIndex)
{
	m_pnResourceTypes[nIndex] = nResourceType;
	m_ppd3dTextures[nIndex] = ::CreateTextureResourceFromDDSFile(pd3dDevice, pd3dCommandList, pszFileName, &m_ppd3dTextureUploadBuffers[nIndex], D3D12_RESOURCE_STATE_GENERIC_READ/*D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE*/);
}

ID3D12Resource* CTexture2::CreateTexture(ID3D12Device* pd3dDevice, UINT nWidth, UINT nHeight, DXGI_FORMAT dxgiFormat, D3D12_RESOURCE_FLAGS d3dResourceFlags, D3D12_RESOURCE_STATES d3dResourceStates, D3D12_CLEAR_VALUE* pd3dClearValue, UINT nResourceType, UINT nIndex)
{
	m_pnResourceTypes[nIndex] = nResourceType;
...
void CTexture2::LoadBuffer(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, void* pData, UINT nElements, UINT nStride, DXGI_FORMAT ndxgiFormat, UINT nIndex)
{
	m_pnResourceTypes[nIndex] = RESOURCE_BUFFER;
	m_pdxgiBufferFormats[nIndex] = ndxgiFormat;
	m_pnBufferElements[nIndex] = nElements;
...
D3D12_UNORDERED_ACCESS_VIEW_DESC CTexture2::GetUnorderedAccessViewDesc(int nIndex)
{
	ID3D12Resource* pShaderResource = GetResource(nIndex);
	D3D12_RESOURCE_DESC d3dResourceDesc = pShaderResource->GetDesc();
...
{
	BoundingBox xmbbModel = m_pMesh->GetBBModel();
...
BoundingBox CGameObject::XMBBModel() const
{
	return m_pMesh->GetBBModel();
}

bool CGameObject::IsInMap(BoundingBox& xmbbMap)
{
	BoundingBox xmbbWorld = m_pMesh->GetBBModel();
...
CSkyBox::CSkyBox(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature) : CGameObject(1)
{
	...
	pSkyBoxShader->CreateCbvSrvUavDescriptorHeaps(pd3dDevice, 0, 1, 0);
	pSkyBoxShader->CreateShaderResourceViews(pd3dDevice, pSkyBoxTexture, 0, GraphicsRoot_SkyBox);
...
CHeightMapTerrain::CHeightMapTerrain(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, LPCTSTR pFileName, int nWidth, int nLength, int nBlockWidth, int nBlockLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color) : CGameObject(1)
{
	...
	CreateShaderVariables(pd3dDevice, pd3dCommandList);
	m_ppMeshes[0]->SetBBModel(BoundingBox(XMFLOAT3(m_nWidth * m_xmf3Scale.x / 2, MAX_MAP_HEIGHT / 2, m_nLength * m_xmf3Scale.z / 2),
		XMFLOAT3(m_nWidth * m_xmf3Scale.x / 2 - 100, MAX_MAP_HEIGHT / 2, m_nLength * m_xmf3Scale.z / 2 - 100)));
	...
	pTerrainTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/terrain.dds", RESOURCE_TEXTURE2D, 2);
	...
	pTerrainShader->CreateCbvSrvUavDescriptorHeaps(pd3dDevice, 0, 3, 0);
	pTerrainShader->CreateShaderResourceViews(pd3dDevice, pTerrainTexture, 0, GraphicsRoot_Terrain);
