...
public:
	BoundingBox&						GetBBModel();
	BoundingBox							GetBBModel() const;
	void SetBBModel(const BoundingBox& BBModel);
};
...
class CExplosionBillboardMesh : public CMesh
{
public:
	CExplosionBillboardMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fWidth, float fHeight, float x = 0, float y = 0);
	virtual ~CExplosionBillboardMesh();
};
...
class CWaterGridMesh : public CMesh
{
protected:
	int							m_nWidth;
	int							m_nLength;
...
public:
	CWaterGridMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, int xStart, int zStart, int nWidth, int nLength, XMFLOAT3 xmf3Scale = XMFLOAT3(1.0f, 1.0f, 1.0f), XMFLOAT4 xmf4Color = XMFLOAT4(1.0f, 1.0f, 0.0f, 0.0f));
