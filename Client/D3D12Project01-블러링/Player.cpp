...
{
	BoundingBox xmbbWorld = m_pChild->m_pMesh->GetBBModel();
...
CAirplanePlayer::CAirplanePlayer(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature)
{
	...
	m_pShader->CreateCbvSrvUavDescriptorHeaps(pd3dDevice, 0, 2,0); //Mi24(1)
...
{
	float RotateSpeed = fTimeElapsed * (m_iSpeedWhirl + 30.f) / MAX_WHIRL_SPEED;
	RotateSpeed = RotateSpeed < fTimeElapsed ? RotateSpeed : fTimeElapsed;

	if (m_pMainRotorFrame)
	{
		XMMATRIX xmmtxRotate = XMMatrixRotationY(XMConvertToRadians(360.0f * 2.0f) * RotateSpeed);
		m_pMainRotorFrame->m_xmf4x4Transform = Matrix4x4::Multiply(xmmtxRotate, m_pMainRotorFrame->m_xmf4x4Transform);
	}
	if (m_pTailRotorFrame)
	{
		XMMATRIX xmmtxRotate = XMMatrixRotationX(XMConvertToRadians(360.0f * 4.0f) * RotateSpeed);
...
void CAirplanePlayer::SetTerrain(void* pContext)
{
	CHeightMapTerrain* pTerrain = (CHeightMapTerrain*)pContext;
	//SuperCobra
	SetPosition(XMFLOAT3(350.0f, 55.0f, 250.0f));

	//Gunship
	//SetPosition(XMFLOAT3(350.0f, 55.0f, 1880.0f));
...
void CAirplanePlayer::OnPlayerUpdateCallback(float fDeltaTime)
{
	...
	XMFLOAT3 patch[25];
	for (int i = 0; i < 25; ++i) {
		patch[i].x = xmf3PlayerPosition.x + (2 * (i % 5) - 4) * xmf3Scale.x;
		patch[i].z = xmf3PlayerPosition.z + (2 * (i / 5) - 4) * xmf3Scale.z;
