...
float* BernsteinCoeffcient5x5(float t)
{
	float fBernstein[5];
...
XMFLOAT3 CubicBezierSum5x5(XMFLOAT3 patch[25])
{
	float fB[5];

	fB[0] = pow(0.5f, 4);
	fB[1] = 4.0f * pow(0.5f, 4);
	fB[2] = 6.0f * pow(0.5f, 4);
	fB[3] = 4.0f * pow(0.5f, 4);
	fB[4] = pow(0.5f, 4);


	XMFLOAT3 xmf3Sum = XMFLOAT3(0.0f, 0.0f, 0.0f);
	for (int i = 0; i < 25; ++i) {
		xmf3Sum = Vector3::Add(xmf3Sum, Vector3::ScalarProduct(patch[i], fB[i % 5] * fB[(i / 5) % 5], false));
	}
	return(xmf3Sum);
}
