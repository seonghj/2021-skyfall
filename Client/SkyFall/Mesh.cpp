...
XMFLOAT3 CHeightMapImage::GetHeightMapTangent(int x, int z)
{
	...
	float y1 = (float)m_pHeightMapPixels[nHeightMapIndex] * m_xmf3Scale.y;
	float y2 = (float)m_pHeightMapPixels[nHeightMapIndex + xHeightMapAdd] * m_xmf3Scale.y;
	XMFLOAT3 xmf3Tangent = XMFLOAT3(m_xmf3Scale.x, y2 - y1, 0.0f);
...
CHeightMapGridMesh::CHeightMapGridMesh(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, int xStart, int zStart, int nWidth, int nLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color, void *pContext) : CMesh(pd3dDevice, pd3dCommandList)
{
	...
	for (int i = 0, z = zStart; z < (zStart + nLength); z++)
	{
		for (int x = xStart; x < (xStart + nWidth); x++, i++)
		{
			...
			m_pxmf3Normals[i].x *= m_xmf3Scale.x;
			m_pxmf3Normals[i].y *= m_xmf3Scale.y;
			m_pxmf3Normals[i].z *= m_xmf3Scale.z;
			m_pxmf3Tangents[i] = pHeightMapImage->GetHeightMapTangent(x, z);
			m_pxmf3Tangents[i].x *= m_xmf3Scale.x;
			m_pxmf3Tangents[i].y *= m_xmf3Scale.y;
			m_pxmf3Tangents[i].z *= m_xmf3Scale.z;
...
{
	D3D12_VERTEX_BUFFER_VIEW pVertexBufferViews[6] = { m_d3dPositionBufferView, m_d3dColorBufferView, m_d3dTextureCoord0BufferView, m_d3dTextureCoord1BufferView, m_d3dNormalBufferView,m_d3dTangentBufferView };
...
void CStandardMesh::LoadMeshFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, FILE* pInFile)
{
	...
	for (; ; )
	{
		...
		{
			m_nVertices = ::ReadUnsignedIntegerFromFile(pInFile);
			/*_stprintf_s(pstrDebug, 256, _T("nVertices: %d\n"), m_nVertices);
		...
		else if (!strcmp(pstrToken, "<UVs>:"))
		{
			...
			/*_stprintf_s(pstrDebug, 256, _T("nUVsPerVertex: %d\n"), (int)nUVsPerVertex);
			OutputDebugString(pstrDebug);*/
			if (nUVsPerVertex > 0) {
				...
				/*for (int i = 0; i < nUVs; ++i) {
					_stprintf_s(pstrDebug, 256, _T("UVs[%d] : (%f, %f)\n"), (int)i,(float)m_pxmf2TextureCoords0[i].x, (float)m_pxmf2TextureCoords0[i].y);
					OutputDebugString(pstrDebug);
				}*/
				m_pd3dTextureCoord0Buffer = ::CreateBufferResource(pd3dDevice, pd3dCommandList, m_pxmf2TextureCoords0, sizeof(XMFLOAT2) * nUVs, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dTextureCoord0UploadBuffer);

				m_d3dTextureCoord0BufferView.BufferLocation = m_pd3dTextureCoord0Buffer->GetGPUVirtualAddress();
				m_d3dTextureCoord0BufferView.StrideInBytes = sizeof(XMFLOAT2);
				m_d3dTextureCoord0BufferView.SizeInBytes = sizeof(XMFLOAT2) * nUVs;
			}
		}
		else if (!strcmp(pstrToken, "<Normals>:"))
		{
			int nNormalsPerVertex = ::ReadIntegerFromFile(pInFile);
			/*_stprintf_s(pstrDebug, 256, _T("nNormalsPerVertex: %d\n"), (int)nNormalsPerVertex);
			OutputDebugString(pstrDebug);*/
			if (nNormalsPerVertex > 0) {
				...
				m_pd3dNormalBuffer = ::CreateBufferResource(pd3dDevice, pd3dCommandList, m_pxmf3Normals, sizeof(XMFLOAT3) * nNormals, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dNormalUploadBuffer);
		...
		else if (!strcmp(pstrToken, "<Tangents>:"))
		{
			...
			/*_stprintf_s(pstrDebug, 256, _T("nTangentsPV: %d \n"), (int)nTangentsPerVertex);
			OutputDebugString(pstrDebug);*/
			if (nTangentsPerVertex > 0) {
				...
				m_pd3dTangentBuffer = ::CreateBufferResource(pd3dDevice, pd3dCommandList, m_pxmf3Tangents, sizeof(XMFLOAT3) * nTangents, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dTangentUploadBuffer);
		...
		else if (!strcmp(pstrToken, "<BiTangents>:"))
		{
			...
			/*_stprintf_s(pstrDebug, 256, _T("nBiTangentsPV: %d\n"), (int)nBiTangentsPerVertex);
			OutputDebugString(pstrDebug);*/
			if (nBiTangentsPerVertex > 0) {
				...
				m_pd3dBiTangentBuffer = ::CreateBufferResource(pd3dDevice, pd3dCommandList, m_pxmf3BiTangents, sizeof(XMFLOAT3) * nBiTangents, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dBiTangentUploadBuffer);
...
CUIMesh::CUIMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float left, float bottom, float right, float top, float alpha)
{
	...
	pVertex[0] = CUIVertex(XMFLOAT3(left, bottom, 0),XMFLOAT2(0,0),alpha);
	pVertex[1] = CUIVertex(XMFLOAT3(right, bottom, 0), XMFLOAT2(1, 0), alpha);
	pVertex[2] = CUIVertex(XMFLOAT3(left, top, 0), XMFLOAT2(0, 1), alpha);
	pVertex[3] = CUIVertex(XMFLOAT3(left, top, 0), XMFLOAT2(0, 1), alpha);
	pVertex[4] = CUIVertex(XMFLOAT3(right, bottom, 0), XMFLOAT2(1, 0), alpha);
	pVertex[5] = CUIVertex(XMFLOAT3(right, top, 0), XMFLOAT2(1, 1), alpha);
